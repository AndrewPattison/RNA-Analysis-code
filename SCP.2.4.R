...
### two curve defaults to False, when True produces two curve which 
### bam files automatically extracted from working directory
...
create_bam_list <- function(select = FALSE){
...
plot_curves <- function(new_list, colour_list, name){
    ...
    axis(1, pos=0, at= c(0,50,100,150,200), tick = 50)
    ...
    # Places the X axis in the middle of the plot
  
    return()
}


plot_mean_legend <- function(bam_list, number_of_replicates, processed_bam_files, legend_list, colour_list){
    mean_division <- tapply(processed_bam_files, (seq_along(processed_bam_files)-1) %/% (length(bam_list)/2), list)
    if(length(bam_list) > 1 & number_of_replicates != length(bam_list)){
        print("check")
        ...
        legend('topleft', bty = "n", inset= c(group_mean1*0.0005,-0.04), legend = paste('D=', paste(round(group_mean1-group_mean2,2)), '±', paste(round( se1+se2,2))))
    ...
    text(110,-18, labels = 'Poly A Tail Length')
    text(-35,50, labels = 'Percent Population (%)', srt =90)
    return()
}
...
SCP <- function(gff, name, number_of_replicates = 2, combine = TRUE, two_curve = FALSE, save = FALSE, select = FALSE){
    ...
    plot_mean_legend(bam_list, number_of_replicates, processed_bam_files, legend_list, colour_list)
    dev.off()
}
...
gff_gene_finder <- function(gff, name){
    ...
    #index1 <- data.frame() 
   # if (grep('peak',name, ignore.case = T) == T){
     # print('check')
