...
### !to use!:
### SCP(gff, name, number_of_replicates = 2, combine = TRUE, two_curve = FALSE, save = FALSE, select = FALSE, plot_mean = TRUE, plot_legend = TRUE)
### gff <- gff file
...
create_bam_list <- function(select){
...
SCP <- function(gff, name, number_of_replicates = 2, combine = TRUE, two_curve = FALSE, save = FALSE, select = FALSE, plot_mean = TRUE, plot_legend = TRUE){
    library(Rsamtools)
    # Write plot output to file
    plot_save(name, save)
    bam_list <- create_bam_list(select)
